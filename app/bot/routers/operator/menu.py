...
_DM_BLOCKED_HINT = "⚠️ Не могу написать вам в личку — напишите боту /start в личных сообщениях, затем повторите"


async def _send_to_dm(message: Message, bot: Bot, telegram_id: int, text: str, reply_markup=None) -> None:
    try:
        await bot.send_message(telegram_id, text, reply_markup=reply_markup)
    except Exception:
        await message.reply(_DM_BLOCKED_HINT)
...
async def group_free_orders(message: Message, session: AsyncSession, bot: Bot, user: User):
    ...
    await _send_to_dm(message, bot, user.telegram_id, text, reply_markup=kb)
...
async def group_view_order(
    ...
    is_admin_or_owner = user.role in (UserRole.admin, UserRole.owner)
    if not is_admin_or_owner and order.operator_id is not None and order.operator_id != user.id:
        await callback.answer("⛔ Это заявка другого оператора", show_alert=True)
        return

    text = format_order_card(order, is_admin=is_admin_or_owner, viewer_id=user.id)
...
async def dm_view_order(
    ...
    from app.db.models.user import UserRole
    is_admin_or_owner = user.role in (UserRole.admin, UserRole.owner)
    # Operators may only view free orders or their own assigned orders
    if not is_admin_or_owner and order.operator_id is not None and order.operator_id != user.id:
        await callback.answer("⛔ Это заявка другого оператора", show_alert=True)
        return
