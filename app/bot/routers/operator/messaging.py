...
async def start_send_requisites(
    ...
    if current_state is not None:
        await callback.answer(
            "⚠️ Сначала завершите текущее действие (/done или /cancel)",
            show_alert=True,
        )
        return
    ...
    saved_requisites = await SettingsService(session=session).get("payment_requisites")
    saved_hint = (
        f"\n\nТекущие реквизиты из настроек:\n<pre>{_html.escape(saved_requisites)}</pre>"
        if saved_requisites
        else "\n\nВ настройках сервиса реквизиты пока не указаны."
    )
...
async def send_requisites_done(message: Message, state: FSMContext, session: AsyncSession, user: User, post_commit: list):
    ...
    support_line = f"\n\n❓ Возникли вопросы по оплате — {support}" if support else ""
    ...
    post_commit.append(bot.send_message(
        ...
        f"После оплаты нажмите «Я оплатил»{support_line}",
...
async def negot_cancel_order(
    ...
    client = await UserRepo(session).get_by_id(order.client_id)
    if client:
        support_line = f"\n\nЕсли возникли вопросы или хотите создать новую заявку — {support}" if support else ""
        post_commit.append(bot.send_message(
            client.telegram_id,
            f"❌ К сожалению, заявка №{order.id} отменена оператором{support_line}",
        ))
    post_commit.append(bot.send_message(
