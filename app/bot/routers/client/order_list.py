...
async def client_paid(
    ...
    support = await SettingsService(session=session).get("support_contact")
    support_line = f"\n\nЕсли что-то пошло не так — {support}" if support else ""
    await callback.message.answer(
        "✅ Спасибо! Мы уведомили администратора об оплате\n"
        f"Ожидайте подтверждения — обычно это занимает несколько минут{support_line}"
    )
    await callback.answer()
...
async def add_files_start(
    ...
    await state.update_data(order_id=order.id, files=[], existing_count=len(order.files))
...
async def add_files_photo(message: Message, state: FSMContext):
    await _stage_file(message, state, file_id=message.photo[-1].file_id, file_type="photo")


@router.message(OrderEditStates.waiting_files, F.document, IsClient())
async def add_files_document(message: Message, state: FSMContext):
    await _stage_file(message, state, file_id=message.document.file_id, file_type="document")


async def _stage_file(message: Message, state: FSMContext, file_id: str, file_type: str):
    data = await state.get_data()
    ...
    if existing_count + len(files) >= MAX_FILES:
        ...
        return
    files.append({"file_id": file_id, "file_type": file_type, "caption": message.caption or ""})
    await state.update_data(files=files)
    total = existing_count + len(files)
    await message.answer(f"✅ Файл принят ({total}/{MAX_FILES}) — ещё или /done")
...
async def client_dispute(
    ...
    from app.services.settings_service import SettingsService
    support = await SettingsService(session=session).get("support_contact")
    support_line = f"\n\nИли напишите нам напрямую: {support}" if support else ""
    await callback.message.answer(
        f"⚠️ Спор открыт — администратор получил уведомление и свяжется с вами{support_line}"
    )
    await callback.answer()
...
async def edit_files_unexpected_type(message: Message):
    """Client sent an unsupported file type (video, voice, sticker, text, etc.)."""
    await message.answer(
        "⚠️ Поддерживаются только фото и документы\n"
        "Отправьте файл нужного типа или /done чтобы завершить\n"
        "/cancel — отменить добавление файлов"
    )
