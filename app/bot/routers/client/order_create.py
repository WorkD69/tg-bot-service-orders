...
async def new_order_from_card(
    ...
    max_orders = await SettingsService(session=session).get_int("max_active_orders")
    active_orders = await OrderRepo(session).get_client_active_orders(user.id)
    if len(active_orders) >= max_orders:
        await callback.answer(
            f"❌ У вас уже {max_orders} активных заявок — дождитесь завершения одной из них",
            show_alert=True,
        )
        return
...
async def _ask_comment(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Напишите что важно знать оператору: особые требования, детали задачи, "
        "пожелания по результату — всё это поможет выполнить работу точно\n\n"
...
async def got_budget(
    ...
    auction_minutes = await svc.get_int("auction_timeout_minutes")

    # Limit: no more than max_active_orders active orders per client
    active_orders = await OrderRepo(session).get_client_active_orders(user.id)
    if len(active_orders) >= max_orders:
        await state.clear()
        await message.answer(
    ...
    # UTC-aware auction end time — uses configurable timeout from BotSettings
    auction_end_at = datetime.now(timezone.utc) + timedelta(minutes=auction_minutes)
    ...
    await message.answer(
        "🎉 Заявка принята!\n\n"
        "👀 Операторы уже видят её — как только кто-то откликнется, "
