...
def _validate_config() -> None:
    ...
    if not settings.owner_telegram_id:
        log.warning("STARTUP: OWNER_TELEGRAM_ID not set — set it in .env to auto-promote yourself to owner on /start")
...
def create_app() -> FastAPI:
    app = FastAPI(title="TG Bot — Сервис заявок", lifespan=lifespan)
