...
async def robokassa_callback(
    ...
    # Guard: endpoint is only valid when Robokassa credentials are configured.
    # In manual payment mode (empty ROBOKASSA_LOGIN) this endpoint must be a no-op —
    # otherwise anyone who knows an order_id can forge a valid MD5 signature with an
    # empty pass2 and mark the order as paid without paying.
    from app.config import settings as _s
    if not _s.robokassa_login or not _s.robokassa_pass2:
        logger.warning(
            "Robokassa callback received but Robokassa is not configured (manual payment mode) — rejecting InvId=%s",
            InvId,
        )
